      ...
      <div className="container mx-auto px-4 py-6 sm:py-12">
        ...
        {payPerUsePlans.length > 0 && (
          <section className="mb-8 md:mb-12">
            <div className="bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50 border border-amber-200 rounded-xl p-4 md:p-6 shadow-sm">
              <div className="text-center mb-4 md:mb-6">
                <div className="flex flex-col sm:flex-row items-center justify-center gap-2 mb-3">
                  <span className="text-2xl md:text-3xl">💰</span>
                  <h3 className="text-lg md:text-xl font-semibold text-gray-900 leading-tight">
                    Precisa de apenas 1 análise?
                  </h3>
                </div>
                <p className="text-gray-600 text-sm md:text-base px-2">
                  Pague somente pelo que usar, sem compromisso mensal
                </p>
              </div>
              <div className="max-w-sm md:max-w-md mx-auto">
                {payPerUsePlans.map(plan => (
                  <div key={plan.id} className="bg-white rounded-xl p-4 md:p-6 shadow-md border-2 border-amber-300 relative overflow-hidden">
                    {/* Header com preço em destaque mobile-first */}
                    <div className="text-center mb-4 mt-2 md:mt-0">
                      <h4 className="text-base md:text-lg font-semibold text-gray-900 mb-3">{plan.name}</h4>
                      <div className="bg-gradient-to-r from-amber-100 to-orange-100 rounded-lg p-3 md:p-4 mb-3">
                    ...
                    {/* Features com melhor espaçamento mobile */}
                    ...
                    <div className="space-y-2">
                      <Button
                        ...
                        className="w-full h-11 md:h-12 bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 font-medium text-sm md:text-base shadow-lg hover:shadow-xl transition-all"
                      >
                      ...
                      <p className="text-xs text-center text-gray-500 px-2">
                        💡 Análise completa + assinatura eletrônica incluída
                      </p>
                    </div>
                  </div>
                ))}
        ...
        <section className="mb-16">
          ...
          {loadingPlans ? (
            ...
            <div className="space-y-6 md:grid md:grid-cols-2 xl:grid-cols-3 md:gap-6 md:space-y-0 max-w-7xl mx-auto">
              {subscriptionPlans.map(plan => {
                const isPopular = plan.name.toLowerCase().includes("premium");
                const isCurrent = userSubscription?.plan.id === plan.id;
                let cardClass = "relative transition-all duration-300 hover:shadow-lg w-full";
                let headerGradient = "bg-white";
                let badge: JSX.Element | null = null;
                
                if (isPopular) {
                  cardClass += " border-2 border-blue-500 shadow-xl md:transform md:scale-105 z-10";
                  headerGradient = "bg-gradient-to-r from-blue-500 to-blue-600 text-white";
                  badge = (
                    <div className="absolute -top-2 left-4 md:left-1/2 md:-translate-x-1/2 z-20">
                      <Badge className="bg-gradient-to-r from-orange-500 to-red-500 text-white px-3 py-1 text-xs shadow-lg">🔥 Mais Popular</Badge>
                    </div>
                  );
                } else if (isCurrent) {
                  ...
                  badge = (
                    <div className="absolute -top-2 left-4 md:left-1/2 md:-translate-x-1/2 z-20">
                      <Badge className="bg-green-500 text-white px-3 py-1 text-xs">✅ Plano Atual</Badge>
                    </div>
                  );
                }
                return (
                  <Card key={plan.id} className={cardClass}>
                    {badge}
                    <CardHeader className={`${headerGradient} rounded-t-lg px-4 py-4 md:px-6 md:py-6 ${isPopular ? "text-white" : ""}`}>
                      <div className="text-center">
                        <CardTitle className={`text-base md:text-lg font-bold mb-2 ${isPopular ? "text-white" : "text-gray-900"}`}>{plan.name}</CardTitle>
                        <CardDescription className={`text-xs md:text-sm ${isPopular ? "text-blue-100" : "text-gray-600"}`}>{plan.description}</CardDescription>
                      </div>
                    </CardHeader>
                    <CardContent className="px-4 py-4 md:px-6 md:py-6 flex-1 flex flex-col">
                      {/* Preço em destaque - Mobile First */}
                      <div className="text-center mb-4 md:mb-6">
                        <div className={`text-xl md:text-2xl lg:text-3xl font-bold mb-1 ${isPopular ? "text-blue-600" : "text-gray-900"}`}>
                          {formatPrice(plan.price)}
                        </div>
                        ...
                        <div className="inline-flex items-center gap-2 bg-gray-50 px-3 py-2 rounded-full">
                          ...
                          <div className="text-xs text-gray-600">
                            análises{plan.max_analyses === -1 ? " ilimitadas" : ""}
                          </div>
                        </div>
                      </div>
                      
                      {/* Features - Mobile otimizado */}
                      <div className="flex-1">
                        <ul className="space-y-2 md:space-y-3">
                          {plan.features.map((f, i) => (
                            <li key={i} className="flex items-start gap-2">
                              <span className="text-green-500 mt-0.5 text-sm flex-shrink-0">✓</span>
                              <span className="text-xs md:text-sm text-gray-700 leading-relaxed">{f}</span>
                            </li>
                          ))}
                        </ul>
                      </div>
                      
                      {/* Badge de economia - Mobile friendly */}
                      {isPopular && (
                        <div className="mt-4 p-2 md:p-3 bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-lg">
                          <div className="flex items-center gap-2 text-center md:text-left">
                            <span className="text-green-600">💰</span>
                            <span className="text-xs md:text-sm font-medium text-green-800">Economia de até 60% vs. avulso</span>
                          </div>
                        </div>
                      )}
                    </CardContent>
                    <CardFooter className="px-4 pb-4 md:px-6 md:pb-6">
                      <div className="w-full space-y-2">
                        <Button
                          ...
                          className={`w-full h-11 md:h-12 text-sm md:text-base font-medium transition-all ${
                            isPopular
                        ...
                        >
                          {processingPayment === plan.id ? (
                            ...
                            <div className="flex items-center gap-2">
                              <span>✅</span>
                              <span className="text-sm md:text-base">Plano Ativo</span>
                            </div>
                          ) : (
                            <div className="flex items-center justify-center gap-2">
                              <span>🚀</span>
                              <span className="text-sm md:text-base">{isPopular ? "Escolher Premium" : "Assinar Agora"}</span>
                            </div>
                          )}
                        </Button>
                        {isPopular && !isCurrent && (
                          <p className="text-xs text-center text-gray-500">⏰ Oferta por tempo limitado</p>
                        )}
                      </div>
                    </CardFooter>
                  </Card>
